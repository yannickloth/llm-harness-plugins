...
│  additionalContext with cached response + staleness warning. │
│  (PreToolUse hooks cannot replace tool output; cache hit is  │
│   injected as context alongside normal tool result.)          │
└──────────────────────────────────────────────────────────────┘
    │ (miss → normal execution)
    ▼
┌──────────────────────────────────────────────────────────────┐
│ Tool Response (PostToolUse)                                   │
│  Write/Edit tool responses:                                   │
│   - cache-store.sh: store prompt-response pair                │
│   - cache-invalidate-files.sh: invalidate entries referencing │
...
├── bin/
│   └── cachecli                  # Runner script (generated by build.sh)
├── hooks/
│   ├── hooks.json                # PreToolUse → cache-check, PostToolUse → cache-store + invalidate-files, SessionStart → invalidate-stale
│   ├── cache-check.sh            # PreToolUse: check prompt against cache, return additionalContext on hit
│   ├── cache-store.sh            # PostToolUse: store Write/Edit responses
│   ├── cache-invalidate-files.sh # PostToolUse: invalidate entries referencing edited file
...
File-change-based invalidation also runs on PostToolUse (Write/Edit) via `cache-invalidate-files.sh`. Each file write/edti invalidates cache entries that reference the changed file by name.
...
|------|-------|---------|--------|----------|
| PreToolUse | Before Task/Read/Glob/Grep | `Task\|Read\|Glob\|Grep` | `cache-check.sh` | Check prompt in cache; if hit, return `additionalContext` with cached response + staleness warning |
| PostToolUse | After tool execution | `Write\|Edit` | `cache-store.sh` | Store prompt-response pair in cache |
| PostToolUse | After tool execution | `Write\|Edit` | `cache-invalidate-files.sh` | Invalidate entries referencing the edited file |
...
| cache-check.sh | Claude Code PreToolUse hook API | Claude Code plugin spec, additionalContext injection |
| cache-store.sh | Claude Code PostToolUse hook API | Claude Code plugin spec |
| cache-invalidate-files.sh | Claude Code PostToolUse hook API | Claude Code plugin spec |
